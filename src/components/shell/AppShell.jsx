...
export function AppShell({
  ...
  useEffect(() => {
    ...
    const onKey = (event) => event.key === 'Escape' && setDrawerOffen(false)
  ...
  return (
    <div className="min-h-dvh bg-canvas">
      ...
      <nav aria-label="Hauptnavigation (mobil)" className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-surface/95 backdrop-blur md:hidden">
          ...
          {primaer.map((item) => (
            <li key={item.id}>
              <NavLink
                ...
                className={({ isActive }) => cn('relative flex flex-col items-center gap-1 py-2.5 text-[0.625rem] font-medium transition-colors', isActive ? 'text-brand-ink' : 'text-ink-3')}
              >
      ...
      {drawerOffen ? (
        <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true" aria-label="Alle Bereiche">
          ...
          <div id="mobile-app-menu" className="absolute inset-x-0 bottom-0 max-h-[85vh] overflow-hidden rounded-t-2xl border-t border-line bg-surface animate-rise">
            <div className="flex items-center justify-between border-b border-line px-4 py-3">
              ...
              <button
                ...
                className="rounded-lg p-2 text-ink-3 hover:bg-surface-muted hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
              >
                ...
                {[...primaer, ...weitere].map((item) => (
                  <li key={item.id}>
                    <NavLink
                      ...
                      onClick={() => setDrawerOffen(false)}
                      className={({ isActive }) => cn('flex items-center gap-2.5 rounded-lg border px-3 py-3 text-[0.8125rem] font-medium', isActive ? 'border-brand-border bg-brand-soft text-brand-ink' : 'border-line text-ink-2')}
                    >

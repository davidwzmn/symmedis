...
export function LaunchReadinessCard() {
  ...
  const orderedGates = useMemo(() => {
    const order = ['leaked_password_protection', 'custom_smtp', 'real_invite_delivery', 'real_magic_link_login', 'restore_drill']
    return [...(state.data?.gates || [])].sort((a, b) => order.indexOf(a.key) - order.indexOf(b.key))
  }, [state.data])
  ...
  return <Card className="border-brand-border">
    ...
    <CardBody className="space-y-4">
      ...
      {data ? <Banner toneName={data.ready ? 'ok' : 'warn'} icon={data.ready ? IconCheck : IconClock} title={data.ready ? 'Externe Launch-Gates vollständig' : 'Launch-Gate bleibt geschlossen'}>{data.ready ? 'Alle externen Produktionsnachweise sind dokumentiert.' : 'SMTP und Restore benötigen explizite reale Nachweise. Invite-Zustellung und E-Mail-Login werden ausschließlich automatisch aus einer echten Kunden-Session verifiziert.'}</Banner> : null}
    </CardBody>
  </Card>
}

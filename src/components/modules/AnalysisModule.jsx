    ...
    <div className="min-w-0 space-y-5">
      {rolle === 'kunde' && nichtFreigegeben > 0 ? (
        <Banner toneName="info" icon={IconLock} title="Analyse in Prüfung">
          {nichtFreigegeben} von {kunde.analyse.length} Punkten werden aktuell noch vom SYMMEDIS-Team geprüft und erscheinen hier, sobald sie freigegeben sind.
        </Banner>
      ) : null}
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_20rem]">
        <Card>
          <CardHeader
            ...
            action={<Segmented size="sm" label="Analyse filtern" value={filter} onChange={setFilter} options={[{ value: 'alle', label: 'Alle' }, { value: 'kritisch', label: 'Auffällig' }, ...(rolle === 'intern' ? [{ value: 'offen', label: 'Offen' }] : [])]} />}
          />
          <CardBody className="px-0 py-0">
            {sortiert.length === 0 ? (
              <EmptyState icon={IconLock} title="Noch keine Ergebnisse freigegeben" description="Sobald das SYMMEDIS-Team die Bewertung geprüft hat, erscheinen die Punkte hier." />
            ) : (
                ...
                {sortiert.map((eintrag) => {
                  ...
                  return (
                    <li key={eintrag.kategorieId}>
                      <button type="button" onClick={() => setAuswahl(eintrag.kategorieId)} className="w-full px-4 py-3.5 text-left transition-colors hover:bg-surface-muted sm:px-5">
                        <div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><ScoreBar score={eintrag.score} label={kategorie.label} sublabel={eintrag.beobachtung} /></div></div>
                        <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                          <Chip size="sm" toneName={PRIORITAETEN[eintrag.prioritaet].tone}>Priorität {PRIORITAETEN[eintrag.prioritaet].label}</Chip>
        ...
        <div className="min-w-0 space-y-5">
          <Card><CardHeader title="Analyseprofil" subtitle="Alle zehn Dimensionen im Vergleich" /><CardBody><RadarChart axes={achsen} series={serie} compare={median} /></CardBody></Card>
          <Card>
...
function AnalyseDetail({ kunde, kategorieId, rolle, onClose }) {
  ...
  const freigabeSetzen = async (next, erfolg = null) => {
    ...
    try {
      ...
      if (!gespeichert) {
        toast.show({ title: 'Freigabe nicht gespeichert', description: 'Der vorherige Freigabestatus wurde wiederhergestellt.', variant: 'danger' })
        return false
      }
  ...
  return (
    <Drawer
      ...
      footer={bearbeitbar ? (
        <div className="flex flex-wrap items-center justify-between gap-3" aria-busy={freigabeSaving}>
          <p className="text-xs text-ink-3" role="status" aria-live="polite">{freigabeSaving ? 'Freigabe wird gespeichert …' : 'Interne Notizen erscheinen nie im Kundenportal.'}</p>
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" disabled={freigabeSaving} onClick={() => freigabeSetzen('intern', { title: 'Intern freigegeben', variant: 'success' })}>{freigabeSaving ? 'Speichert …' : 'Intern freigeben'}</Button>
            <Button size="sm" disabled={freigabeSaving} onClick={() => freigabeSetzen('kunde', { title: 'Für Kunden freigegeben', description: `${kategorie.label} ist jetzt im Kundenportal sichtbar.`, variant: 'success' })}>{freigabeSaving ? 'Speichert …' : 'Für Kunden freigeben'}</Button>
          </div>
        </div>
    ...
    >
      <div className="space-y-5 px-5 py-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {rolle === 'intern' ? <Chip toneName={FREIGABE[eintrag.freigabe].tone} icon={eintrag.sichtbarKunde ? IconEye : IconEyeOff}>{FREIGABE[eintrag.freigabe].label}</Chip> : null}
        </div>

        <div className="rounded-lg border border-line bg-surface-muted p-4">
          <div className="flex items-baseline justify-between"><span className="text-xs text-ink-2">Reifegrad</span><span className="tabular text-2xl font-semibold text-ink">{eintrag.score}</span></div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-viz-track"><div className={cn('h-full rounded-full', tone(stufe.tone).bar)} style={{ width: `${eintrag.score}%` }} /></div>
          <p className="mt-2 text-xs text-ink-3">0–39 kritisch · 40–57 auffällig · 58–74 solide · ab 75 stark</p>
        </div>
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Select label="Freigabestatus" value={eintrag.freigabe} disabled={freigabeSaving} onChange={(e) => freigabeSetzen(e.target.value)}>
                {Object.entries(FREIGABE).map(([key, wert]) => <option key={key} value={key}>{wert.label}</option>)}
              </Select>
            </div>
        ...
        {rolle === 'kunde' ? (
          <div className="rounded-lg border border-brand-border bg-brand-soft p-4">
            <h3 className="flex items-center gap-2 text-xs font-semibold text-brand-ink"><IconCheckCircle className="size-3.5" />Menschlich geprüft</h3>
            <p className="mt-1.5 text-[0.8125rem] leading-relaxed text-ink-2">Diese Bewertung wurde durch die Software strukturiert und anschließend vom SYMMEDIS-Team geprüft und freigegeben.</p>
          </div>
        ) : null}
      ...
      {kunde.bremsen.map((bremse) => {
        ...
        return (
          <Card key={bremse.id} className="flex flex-col p-4">
            <div className="flex items-start justify-between gap-2"><span className="inline-flex size-7 items-center justify-center rounded-lg bg-surface-inverse text-xs font-semibold text-canvas">{bremse.rang}</span><Chip size="sm" toneName={prio.tone} icon={bremse.prioritaet === 'hoch' ? IconAlert : undefined}>{prio.label}</Chip></div>
            <h2 className="mt-3 text-[0.9375rem] font-semibold text-ink">{bremse.titel}</h2>
            <p className="mt-1.5 flex-1 text-[0.8125rem] leading-relaxed text-ink-2">{bremse.beschreibung}</p>
            <div className="mt-3 flex items-center gap-2 text-xs"><Chip size="sm" toneName={stufe.tone}>Score {bremse.score}</Chip><span className="text-ink-3">{KATEGORIE_MAP[bremse.kategorieId].gruppe}</span></div>
            {!kompakt ? <div className="mt-3 border-t border-line pt-3"><p className="text-xs font-medium text-ink-2">Empfohlene nächste Aktion</p><p className="mt-1 text-[0.8125rem] leading-snug text-ink">{bremse.naechsteAktion}</p></div> : null}
            {onOeffnen ? <Button variant="ghost" size="sm" className="mt-3 self-start" onClick={() => onOeffnen(bremse.kategorieId)}>Details ansehen</Button> : null}
          </Card>
        )
      })}
    </div>

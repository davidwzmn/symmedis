  ...
  const weiter = async (aufgabe) => {
    ...
    try {
      ...
      toast.show({
        ...
        description: error instanceof Error ? error.message : 'Die Änderung wurde zurückgesetzt. Bitte versuchen Sie es erneut.',
        variant: 'danger',
      })
    } finally {
      setSpeichert(null)
    ...
    <div className="space-y-5">
      {zaehler.ueberfaellig > 0 ? (
        <Banner toneName="urgent" icon={IconClock} title="Überfällige Aufgaben">
          ...
          {zaehler.ueberfaellig === 1 ? 'Aufgabe ist' : 'Aufgaben sind'} über den Fälligkeitstermin hinaus. Sie blockieren die nachfolgenden Schritte im 90-Tage-Plan.
        </Banner>
      ) : null}

      <Card>
        <CardBody className="space-y-4">
          <StatusSplit segments={[
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          <Segmented label="Aufgaben filtern" value={filter} onChange={setFilter} options={[
            ...
            ...(zaehler.ueberfaellig ? [{ value: 'ueberfaellig', label: 'Überfällig', count: zaehler.ueberfaellig }] : []),
          ]} />
        </div>
        <div className="flex items-center gap-2">
          <SearchInput value={suche} onChange={(e) => setSuche(e.target.value)} placeholder="Aufgabe suchen …" label="Aufgaben durchsuchen" className="flex-1 sm:w-56" />
          <Segmented label="Ansicht wählen" value={ansicht} onChange={setAnsicht} options={[
      ...
      {gefiltert.length === 0 ? (
        <Card><EmptyState icon={IconCheckSquare} title="Keine Aufgaben in dieser Auswahl" description="Setzen Sie den Filter zurück, um alle Aufgaben des Projekts zu sehen." action={<Button variant="secondary" size="sm" onClick={() => { setFilter('alle'); setSuche('') }}>Filter zurücksetzen</Button>} /></Card>
      ) : ansicht === 'board' ? (
          ...
          {SPALTEN.map((spalte) => {
            ...
            return (
              <div key={spalte.id} className="rounded-card border border-line bg-surface-muted p-3">
                <div className="mb-3 flex items-center justify-between"><p className="text-[0.8125rem] font-semibold text-ink">{spalte.label}</p><span className="tabular text-xs text-ink-3">{eintraege.length}</span></div>
                <ul className="space-y-2.5">
                  {eintraege.map((aufgabe) => <li key={aufgabe.id}><AufgabenKarte aufgabe={aufgabe} onWeiter={schreibbar ? weiter : undefined} pending={speichert === aufgabe.id} disabled={Boolean(speichert)} /></li>)}
                  {eintraege.length === 0 ? <li className="rounded-lg border border-dashed border-line-strong px-3 py-6 text-center text-xs text-ink-3">Keine Aufgaben</li> : null}
                </ul>
              </div>
            )
          })}
            ...
            {gefiltert.map((aufgabe) => {
              ...
              return (
                <li key={aufgabe.id} className="flex items-start gap-3 px-4 py-3.5 sm:px-5">
                  <button
                    type="button"
                    disabled={!schreibbar || Boolean(speichert)}
                    onClick={() => weiter(aufgabe)}
                    aria-busy={pending || undefined}
                    aria-label={aufgabe.status === 'erledigt' ? `${aufgabe.titel} wieder öffnen` : `${aufgabe.titel} weiterschalten`}
                    className={cn(
                      'mt-0.5 inline-flex size-5 shrink-0 items-center justify-center rounded border transition-colors',
                      aufgabe.status === 'erledigt' ? 'border-ok-ink bg-ok-ink text-on-solid' : aufgabe.status === 'in-arbeit' ? 'border-info bg-info-soft text-info-ink' : 'border-line-strong text-transparent hover:border-brand',
                      (!schreibbar || speichert) && 'cursor-not-allowed opacity-60',
                    )}
                  >
                    {pending ? <Spinner /> : aufgabe.status === 'erledigt' ? <IconCheck className="size-3.5" /> : aufgabe.status === 'in-arbeit' ? <span className="size-2 rounded-full bg-info" /> : null}
                  </button>

                  <div className="min-w-0 flex-1">
                    <p className={cn('text-[0.875rem] leading-snug font-medium', aufgabe.status === 'erledigt' ? 'text-ink-3 line-through' : 'text-ink')}>{aufgabe.titel}</p>
                    <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                      <Chip size="sm" toneName={prio.tone}>{prio.label}</Chip>
                      <Chip size="sm" toneName={faellt.tone} icon={faellt.ueberfaellig ? IconClock : undefined}>{faellt.label}</Chip>
                      ...
                      <span className="text-xs text-ink-3">{KATEGORIE_MAP[aufgabe.kategorieId]?.label} · Messgröße: {aufgabe.kpi}</span>
                    </div>
                  </div>
                  <span className="hidden shrink-0 text-xs text-ink-3 lg:block">{aufgabe.zustaendig}</span>
                </li>
              )
            })}
          </ul>
  ...
  return (
    <div className="rounded-lg border border-line bg-surface p-3 shadow-xs">
      ...
      <div className="mt-2.5 flex items-center justify-between gap-2 border-t border-line pt-2.5">
        ...
        {onWeiter ? <Button variant="ghost" size="xs" onClick={() => onWeiter(aufgabe)} disabled={disabled} aria-busy={pending || undefined}>{pending ? <Spinner /> : null}{aufgabe.status === 'erledigt' ? 'Öffnen' : 'Weiter'}</Button> : null}
      </div>
    </div>
  )
}
...
export function AufgabenVorschau({ kunde, anzahl = 4, nurRolle, onAlle }) {
  const offen = kunde.aufgaben.filter((a) => a.status !== 'erledigt').filter((a) => !nurRolle || a.verantwortlich === nurRolle).sort((a, b) => tageBis(a.faellig) - tageBis(b.faellig)).slice(0, anzahl)
  return (
    <Card>
      ...
      <CardBody className="px-0 py-0">
        {offen.length === 0 ? <EmptyState compact icon={IconCheck} title="Nichts offen" description="Alle Aufgaben in diesem Bereich sind erledigt." /> : (
          <ul className="divide-y divide-line">
            {offen.map((aufgabe) => {
              const faellt = faelligkeit(aufgabe)
              return <li key={aufgabe.id} className="flex items-start gap-3 px-4 py-3 sm:px-5"><span aria-hidden="true" className={cn('mt-1.5 size-2 shrink-0 rounded-full', faellt.ueberfaellig ? 'bg-urgent' : 'bg-neutral')} /><span className="min-w-0 flex-1"><span className="block text-[0.8125rem] leading-snug font-medium text-ink">{aufgabe.titel}</span><span className="mt-0.5 block text-xs text-ink-3">{faellt.label} · {aufgabe.zustaendig}</span></span></li>
            })}
          </ul>
        )}
      </CardBody>

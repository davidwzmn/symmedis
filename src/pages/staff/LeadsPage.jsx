...
export function LeadsPage() {
  ...
  const setStatus = async (lead, status) => {
    ...
    try {
      ...
      toast.show({ title: 'Status aktualisiert', description: `${lead.company} · ${STATUS[status]?.label || status}`, variant: 'success' })
    } catch (error) {
                ...
                <CardBody className="p-5 sm:p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="truncate text-base font-semibold tracking-tight text-ink">{lead.company}</p>
                      <p className="mt-1 text-[0.8125rem] text-ink-2">{lead.name} · <a href={`mailto:${lead.email}`} className="font-medium text-brand-ink hover:underline">{lead.email}</a></p>
                    </div>
                    <Chip toneName={state.tone}>{state.label}</Chip>
                  </div>

                  <div className="mt-4 rounded-xl border border-line bg-surface-muted p-4">
                    <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.09em] text-ink-3"><IconTarget className="size-3.5" />Wachstumsfrage</div>
                    <p className="mt-2 whitespace-pre-wrap text-[0.8125rem] leading-relaxed text-ink-2">{lead.situation || 'Kein zusätzlicher Kontext angegeben.'}</p>
                  </div>
                  ...
                  <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <Select label="Bearbeitungsstatus" value={lead.status} onChange={(event) => setStatus(lead, event.target.value)} className="sm:w-44" required>
                      {Object.entries(STATUS).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}
                    </Select>
                  </div>
                </CardBody>

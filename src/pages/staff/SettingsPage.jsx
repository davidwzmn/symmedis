...
const ROLLEN = [
  {
    id: 'admin', name: 'Administrator:in',
    rechte: ['Kunden und Projekte verwalten', 'Analysen freigeben', 'Interne Notizen lesen und schreiben', 'Berichte finalisieren', 'Teamzugänge verwalten'],
  },
  {
    id: 'intern', name: 'Mitarbeiter:in',
    rechte: ['Projektarbeit innerhalb der eigenen SYMMEDIS-Organisation', 'Analyse bearbeiten', 'Interne Notizen lesen und schreiben', 'Kundenfreigaben nach Prozess'],
  },
  {
    id: 'kunde', name: 'Kundenzugang',
    rechte: ['Nur eigener Mandant', 'Freigegebene Analysepunkte lesen', 'Eigene Aufgabenstatus bearbeiten', 'Dokumente im eigenen Projekt hochladen', 'Keine internen Notizen oder Entwürfe'],
  },
]
...
export function SettingsPage() {
  ...
  const { echteAuthentifizierung, accessToken } = useSession()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Interne Konten" value={echteAuthentifizierung ? team.length : '–'} icon={IconUsers} toneName="accent" hint="aus Supabase Auth/Profiles" />
        ...
        <CardBody className="space-y-4">
          <div className="flex flex-wrap gap-2" aria-label="Extension-Kategorien filtern">
            {categories.map((category) => (
              <button key={category} type="button" onClick={() => setFilter(category)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${filter === category ? 'border-accent bg-accent-soft text-accent-ink' : 'border-line bg-surface text-ink-2 hover:bg-surface-muted'}`}>
                {category}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-5">
          <Card>
            ...
            <CardBody className="space-y-4">
              ...
              <Toggle checked={hinweise} onChange={(wert) => { setHinweise(wert); toast.show({ title: wert ? 'Hinweise aktiviert' : 'Hinweise deaktiviert', description: 'Persistente Notification Preferences sind im Backend vorbereitet.' }) }} label="Hinweise im Benachrichtigungszentrum" description="Neue Dokumente, offene Freigaben, überfällige Aufgaben." />
          ...
          <Card>
            ...
            <CardBody className="px-0 py-0">
              {teamLoading ? <p className="px-4 py-5 text-sm text-ink-3 sm:px-5">Team wird geladen …</p> : null}
              {teamError ? <p className="px-4 py-5 text-sm text-danger-ink sm:px-5">{teamError}</p> : null}

...
const ROLLEN = {
  kunde: {
    ...
    ziel: '/portal/uebersicht',
    icon: IconBuilding,
    punkte: [
      'Freigegebene Analyse mit Beleg und Empfehlung',
      'Aufgaben, Dokumente und Termine des eigenen Projekts',
      'Keine internen Notizen, keine fremden Mandanten',
    ],
  },
  intern: {
    ...
    ziel: '/intern/uebersicht',
    icon: IconUsers,
    punkte: [
      'Analyse bearbeiten und stufenweise freigeben',
      'Interne Notizen, Prüfpfad und Teamauslastung',
      'Projektübergreifende Listen und Auswertungen',
    ],
  },
}
        ...
        <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[1fr_1.1fr] lg:gap-12">
          <div>
            ...
            <p className="mt-3 max-w-md text-[0.9375rem] leading-relaxed text-ink-2">
              Das Kundenportal zeigt ausschließlich geprüfte und freigegebene Inhalte. Das
              Mitarbeiterportal enthält zusätzlich den Analyse-Editor, den Freigabeprozess und
              interne Notizen.
            </p>

            <ul className="mt-6 space-y-2.5">
              {aktiv.punkte.map((punkt) => (
                <li key={punkt} className="flex items-start gap-2.5 text-[0.875rem] text-ink">
          ...
          <div className="rounded-2xl border border-line bg-surface p-6 shadow-sm sm:p-8">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        gewaehlt
                          ...
                          : 'border-line-strong hover:border-brand hover:bg-surface-muted',
                      )}
                    >
                      <span
                        className={cn(
                          'inline-flex size-8 items-center justify-center rounded-md',
                          gewaehlt ? 'bg-brand text-on-brand' : 'bg-surface-muted text-ink-2',
                        )}
                      >
                        <Icon className="size-4" />
                      </span>
                      <span className={cn('text-[0.8125rem] font-semibold', gewaehlt ? 'text-brand-ink' : 'text-ink')}>
                        {wert.titel}
                      </span>
            ...
            <p className="mt-4 text-center text-xs leading-relaxed text-ink-3">
              Lieber erst ansehen?{' '}
              <Link to="/demo" className="font-medium text-brand-ink hover:underline">
                Zur Produktdemo ohne Anmeldung
              </Link>
            </p>
          </div>
        </div>

...
export async function fetchPilotValidationReviews(accessToken, projectId) {
  return restSelect(
    ...
    accessToken,
    `select=id,review_round,reviewed_at,understanding_score,decision_clarity_score,human_review_material_change,decision_changed,portal_return_count,willingness_to_pay,renewal_signal,customer_understanding_note,decision_change_note,interviewer_note,created_at,updated_at&project_id=eq.${encodeURIComponent(projectId)}&order=review_round.desc`,
  )
}

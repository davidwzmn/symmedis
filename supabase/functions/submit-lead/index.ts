...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (name.length < 2 || company.length < 2 || !EMAIL_RE.test(email)) {
      return json(req, 422, { error: "Bitte Name, Unternehmen und eine gültige E-Mail-Adresse angeben." });
    }
    ...
    if ((recent || []).length >= MAX_PER_15_MINUTES) {
      return json(req, 429, { error: "Zu viele Anfragen in kurzer Zeit. Bitte versuchen Sie es später erneut." });
    }

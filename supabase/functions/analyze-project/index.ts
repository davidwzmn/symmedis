...
function validateFindings(raw: any[]) {
  ...
  for (const item of raw) {
    ...
    byCategory.set(item.category_id, { category_id: item.category_id, score: Math.max(0, Math.min(100, Math.round(Number(item.score) || 0))), observation: String(item.observation || "").slice(0, 5000), cause: String(item.cause || "").slice(0, 5000), impact: String(item.impact || "").slice(0, 5000), recommendation: String(item.recommendation || "").slice(0, 5000), evidence: String(item.evidence || "").slice(0, 5000), priority: ["hoch", "mittel", "niedrig"].includes(item.priority) ? item.priority : "mittel", confidence: Math.max(0, Math.min(100, Math.round(Number(item.confidence) || 0))), evidence_sources: Array.isArray(item.evidence_sources) ? item.evidence_sources.map((value: unknown) => String(value).slice(0, 300)).slice(0, 12) : [] });
  }
...
Deno.serve(async (req: Request) => {
  ...
  let runId: string | null = null, actorId: string | null = null, auditProjectId: string | null = null, auditClientId: string | null = null, auditOrgId: string | null = null;
  try {
    ...
    const project = projects?.[0]; if (!project) return json(req, 404, { error: "Projekt nicht gefunden." });
    ...
    const client = clients?.[0]; if (!client) return json(req, 404, { error: "Kunde nicht gefunden." });
    ...
    if (!ANTHROPIC_API_KEY) {
      if (runId) await db(auth, `analysis_runs?id=eq.${runId}`, { method: "PATCH", body: JSON.stringify({ status: "failed", error_message: "ANTHROPIC_API_KEY nicht konfiguriert", completed_at: new Date().toISOString() }) });
      await adminInsert("audit_events", { organization_id: auditOrgId, client_id: auditClientId, project_id: projectId, actor_user_id: actorId, event_type: "analysis.failed", entity_type: "analysis_run", entity_id: runId, summary: "KI-Ursachenanalyse nicht konfiguriert", metadata: { reason: "anthropic_key_missing" } }).catch(() => undefined);
      return json(req, 503, { error: "KI-Analyse ist noch nicht konfiguriert. Hinterlegen Sie ANTHROPIC_API_KEY als Supabase Edge-Function-Secret." });
    }
    ...
    if (runId) await db(auth, `analysis_runs?id=eq.${runId}`, { method: "PATCH", body: JSON.stringify({ status: "completed", source_documents: used, finding_count: findings.length, completed_at: new Date().toISOString() }) });
    await adminInsert("audit_events", { organization_id: auditOrgId, client_id: auditClientId, project_id: projectId, actor_user_id: actorId, event_type: "analysis.completed", entity_type: "analysis_run", entity_id: runId, summary: "KI-Ursachenanalyse abgeschlossen – Prüfung erforderlich", metadata: { model: ANTHROPIC_MODEL, source_documents: used, findings: findings.length } }).catch(() => undefined);

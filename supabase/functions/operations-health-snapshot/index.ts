...
function evaluate(summary: ReturnType<typeof aggregate>) {
  ...
  for (const item of summary) {
    ...
    if (item.eventType === "lead_ingress" && item.samples >= 5 && item.failureRate > 0.10) alerts.push({ severity: "SEV-2", signal: label, detail: `failure rate ${(item.failureRate * 100).toFixed(1)}% > 10%` });

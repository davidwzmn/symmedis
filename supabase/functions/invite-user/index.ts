...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { data: caller, error: callerError } = await userClient.from("profiles").select("id,role,organization_id").maybeSingle();
    ...
    if (!clientId || !email.includes("@")) return json(req, 400, { error: "Kunde und gültige E-Mail-Adresse sind erforderlich." });
    ...
    if (clientError || !client) return json(req, 404, { error: "Kunde nicht gefunden." });
    ...
    return json(req, 500, { error: error instanceof Error ? error.message : "Einladung fehlgeschlagen." });
  }
});

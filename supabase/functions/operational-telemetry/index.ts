...
const EVENT_TYPES = new Set(['render_failure', 'workspace_load', 'save_action', 'file_transfer', 'web_vital'])
const SURFACES = new Set(['customer', 'staff', 'unknown'])
const OUTCOMES = new Set(['success', 'failure', 'cancelled', 'observed'])
const OPERATIONS = new Set(['insert', 'update', 'upload', 'download', 'delete'])
...
Deno.serve(async (req: Request) => {
  ...
  if (eventType === 'save_action' && (!operation || !new Set(['insert', 'update']).has(operation))) {
    return json(400, { error: 'invalid_save_operation' })
  }
  if (eventType === 'file_transfer' && (!operation || !new Set(['upload', 'download', 'delete']).has(operation))) {
    return json(400, { error: 'invalid_file_operation' })
  }
  if (!new Set(['save_action', 'file_transfer']).has(eventType) && operation) return json(400, { error: 'operation_not_allowed' })

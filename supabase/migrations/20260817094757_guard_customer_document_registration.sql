...
begin
  ...
  if new.source <> 'kunde' or new.customer_visible is not true then
    raise exception 'Kundendokumente müssen als eigener, kundensichtbarer Upload registriert werden.' using errcode = '42501';
  end if;
...
drop trigger if exists guard_customer_document_registration on public.documents;
create trigger guard_customer_document_registration
before insert on public.documents
for each row
execute function private.guard_customer_document_registration();

drop policy if exists symmedis_project_files_insert on storage.objects;
create policy symmedis_project_files_insert
on storage.objects
for insert
to authenticated
with check (
